...
def dashboard(request):
    ...
    asset_stats = Asset.objects.aggregate(
        ...
        available_assets=Count('id', filter=Q(status='available')),
        assigned_assets=Count('id', filter=Q(status='assigned')),
        maintenance_assets=Count('id', filter=Q(status='maintenance')),
        retired_assets=Count('id', filter=Q(status='retired')),
    )
    total_value = asset_stats.get('total_value') or 0
    ...
    status_data = {
        'available': asset_stats.get('available_assets', 0),
        'assigned': asset_stats.get('assigned_assets', 0),
        'maintenance': asset_stats.get('maintenance_assets', 0),
        'retired': asset_stats.get('retired_assets', 0),
    }
    ...
    status_counts = [
        asset_stats.get('available_assets', 0),
        asset_stats.get('assigned_assets', 0),
        asset_stats.get('maintenance_assets', 0),
        asset_stats.get('retired_assets', 0),
    ]
    ...
    context = {
        ...
        'available_assets': asset_stats.get('available_assets', 0),
        'assigned_assets': asset_stats.get('assigned_assets', 0),
        'maintenance_assets': asset_stats.get('maintenance_assets', 0),
        'retired_assets': asset_stats.get('retired_assets', 0),
...
def asset_detail(request, pk):
    ...
    qr_url = reverse('assets:generate_qr_code', kwargs={'pk': pk})
...
URL: {request.build_absolute_uri(reverse('assets:asset_detail', kwargs={'pk': pk}))}"""
...
def download_qr_code(request, pk):
    ...
    # QR data with URL
    qr_data = request.build_absolute_uri(reverse('assets:asset_detail', kwargs={'pk': pk}))
